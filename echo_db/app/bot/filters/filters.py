...
class LocaleFilter(BaseFilter):
    async def __call__(self, callback: CallbackQuery, locales: list) -> Any:
...
class UserRoleFilter(BaseFilter):
    def __init__(self, *roles: str | UserRole):
        if not roles:
            raise ValueError(
                "At least one role shold be provided to '%s'"
        ...
        if not self.roles:
            raise ValueError(
                "No valid roles provided to '%s'" % (self.__class__.__name__)
            )
