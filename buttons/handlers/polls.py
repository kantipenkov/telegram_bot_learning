...
from commands import CAT_POLL, POLL_EXAMPLE
...
def dump(message: TelegramObject, prefix: str="Created poll"):
    ...
    logger.debug(
        "%s: %s", prefix, message.model_dump_json(indent=4, exclude_none=True)
    )


@router.message(Command(commands=POLL_EXAMPLE))
...
# create cat poll
@router.message(Command(commands=CAT_POLL))
